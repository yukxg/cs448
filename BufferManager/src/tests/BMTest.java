...
class BMDriver extends TestDriver implements GlobalConst {
	...
	protected boolean test1() {
		...
		for (pid.pid = firstPid.pid, lastPid.pid = pid.pid + numPages; status == OK
			...
			try {
				...
				// System.out.println("BMTest: pinPage ```pid is "+pid.pid);
			} catch (Exception e) {
				status = FAIL;
				System.err
				...
				if (status == OK) {
					...
					if (data != (pid.pid) + 99999) {
						...
						System.err.print("*** Read wrong data back from page "
								+ pid.pid +" data is "+data+" should be "+x+ "\n");
					}
				}
	...
	protected boolean test2() {
		...
		if (status == OK) {
			try {
				Minibase.BufferManager.pinPage(firstPid, pg, /* emptyPage: */
						false);
			} catch (Exception e) {
				status = FAIL;
				System.err
	...
	protected boolean test3() {
		...
		for (index = 0; status == OK && index < numPages; ++index) {
			...
			if (status == OK) {
				...
				if (status == OK) {
					if (pid.pid % 20 != 12) {
						try {
							Minibase.BufferManager.unpinPage(pid, /* dirty: */
									true);
						} catch (Exception e) {
							status = FAIL;
							System.err.print("*** Could not unpin dirty page "
		...
		if (status == OK) {
			...
			for (index = 0; status == OK && index < numPages; ++index) {
				...
				if (status == OK) {
					try {
						Minibase.BufferManager.unpinPage(pid, true); // might
																		// not
																		// be
																		// dirty
					} catch (Exception e) {
